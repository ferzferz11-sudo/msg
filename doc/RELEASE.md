...
### Способ 2b: Hermes Gateway из Android-приложения

В приложении Lavender Messenger:
1. Открыть «Агенты» → ⚙ (настройки)
2. Секция «Подключение через шлюз»
3. Ввести:
   - SSH хост: `13.140.25.249` (IP адрес, НЕ SSH alias!)
   - SSH порт: `22`
   - SSH логин: `root`
   - SSH пароль: (ваш пароль)
   - Хост сервера: `localhost` (gRPC на том же сервере)
   - Порт сервера: `50051`
   - Локальный порт: `50052`
4. Нажать «Создать туннель»
5. После создания туннеля — генерировать токен и запускать агента

⚠️ **Важно:** Android не резолвит SSH aliases (например `lava`).
Всегда вводите IP адрес напрямую!

Туннель: `localhost:50052 → 13.140.25.249:50051 через SSH`
