...
| 31 | Concurrency fixes (hub broadcast) | Средняя | Требует анализа contention |
| 32 | DB split (db.go → 4 файла) | Низкая | Читаемость (~1800 строк) |
| 33 | Unified RateLimiter (Redis) | Средняя | Нужен Redis |
| 34 | Удаление deprecated v1 кода (auth, chat) | Низкая | ~500 строк, клиенты не обновились |

### AI v2 — Доработка

| # | Что | Сложность | Причина |
|---|-----|-----------|---------|
| 46 | WebSocket provider (real impl) | Средняя | Нужна gorilla/websocket |
| 47 | MiMo deep integration (DB, bash) | Средняя | Требует анализа безопасности |
