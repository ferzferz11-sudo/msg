...
## Промпт для следующей сессии (feat/1.1.2.x — багфикс AI чатов)

```
Продолжаем работу над Lavender Messenger. Ветка feat/1.1.2.x.

Текущие баги (в работе):
1. Hermes оркестратор — ошибка создания сессии (permission denied в prod DB, исправлено)
2. HermesGrpc — неправильный маппинг proto полей (исправлено, ожидает тестирования)
3. OWL — сообщения не сохраняются в БД (дебаг-логи добавлены, ожидает тестирования)
4. Удаление ИИ чата — удаляются оба (ожидает уточнения)
...
Что делать дальше:
1. Завершить багфикс AI чатов (тестирование → деплой)
2. Выбрать следующую фичу из бэклога
