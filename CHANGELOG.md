...
- **Server:**
  ...
  - **Множество OWL чатов**: Пользователь может создать несколько отдельных OWL чатов с разными моделями и API ключами
  - **Настройки модели**: Выбор модели через `/model` команду или настройки чата (23 модели + произвольная)
  - **Персональный API key**: Подсобственного ключа OpenRouter для обхода rate limits
  - **История сообщений**: История OWL чатов сохраняется в БД и загружается при открытии
  - **Исправлено**: GetChats корректно возвращает OWL чатов по username (было сравнение UUID с строковым полем)

- **Клиент Android:**
  - **OWL интеграция**: Поддержка AI-ассистента в клиенте (стриминг ответов)
  - **Множество OWL чатов**: Создание из меню "+" на экране чатов
  - **Настройки OWL**: API key, выбор модели через диалог настроек (⚙️)
  - **Удаление OWL чатов**: Через меню (🗑️) с подтверждением
  - **Striming**: Ответ появляется по словам в реальном времени
  - **Исправлено**: Ответы не дублируются и не исчезают (partial update через payload)
  - **Исправлено**: OWL чат сразу появляется в списке после создания
  - **История**: Сообщения загружаются с сервера при открытии чата
  - **Версия**: 1.1.0.3
