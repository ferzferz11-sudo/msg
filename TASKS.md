...
### Высокий приоритет
1. **[AI] кнопка в списке чатов (v1.1.1.4)** — см. детальный план ниже
2. **Исправить пути скриптов деплоя** — /deploy искал скрипты в /root/scripts/ (не существовало). Исправлено на /root/msg/scripts/. Нужно проверить что /deploy dev и /prod работают корректно.
3. **APK сборка и тест на устройстве** — полный цикл, проверить все функции
4. **Деплой на prod** — проверить на dev, потом деплой
